...
class AngleSlotsTests(unittest.TestCase):
    def test_default_angles_are_five(self):
        # Per design D-006: 技术内核 / 商业影响 / 用户体验 / 历史类比 / 反对意见
        self.assertEqual(len(DEFAULT_ANGLES), 5)
        self.assertIn("技术内核", DEFAULT_ANGLES)
        self.assertIn("反对意见", DEFAULT_ANGLES)
    ...
    def test_pick_unused_angle_all_used_returns_oldest(self):
        # All 5 used → rotate back to first (rotation policy: oldest-first)
