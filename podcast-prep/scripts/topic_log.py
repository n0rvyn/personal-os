...
def save_topic_log(path, data: dict) -> None:
    p = Path(path)
    # Per plan-verifier S2 finding: ensure parent dir exists (first-run finalize would
    # otherwise FileNotFoundError when {exchange_dir}/podcast-prep/ doesn't exist yet).
