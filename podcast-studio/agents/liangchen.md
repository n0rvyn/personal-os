...
### 4. 最近用过的历史锚（recent_anchors）

从 `recent_bodies` 摘录里，抽出最近几期**反复用过的历史类比锚**（如 `1956苏伊士`、`1973石油禁运`、`古巴导弹危机`）。这个清单交给达芬奇，让它这期**换锚或不用**，避免每期套同一套历史工具箱。

**用素材里出现的具体事件名**（如 `1973石油禁运`、`1956苏伊士危机`），不要抽象成"1970s滞胀""冷战危机"这类概括——达芬奇要拿这个清单去比对它将要搜的历史背景，越具体越挡得住重复。抽不到就空列表。
