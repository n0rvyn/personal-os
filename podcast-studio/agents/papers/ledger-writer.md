...
**`anchor` 是你从原文里「复制粘贴」出来的一段连续字符，不是你「转述 / 概括 / 润色」出来的句子。** 把它想成：你在原文里用鼠标选中一段、Ctrl-C，原样粘进 `anchor` 字段——中间一个字都不能动。`text` 字段才是你自己的转述；`anchor` 必须是原文里一字不差能找到的连续子串。

下游 `verify_anchors` 做的是 `anchor in fulltext`，**只做 whitespace 归一化**（换行 / 多空格压成单空格），除此之外逐字符比对。所以：

- **逐字复制，禁止任何增 / 删 / 改：**
  - **禁止在句首或句尾加词。** 最常见的翻车：原文是 `LVBench, our 7B agent outperforms ...`，你写成 `On LVBench, our 7B agent outperforms ...`——你自己加的那个 `On ` 在那个位置的原文里不存在，门直接判 fabricated、整期停线。**原文那一处以哪个词开头，你的 anchor 就以哪个词开头**，不许补主语 / 介词 / 连接词（On / In / The / 本文 / 我们 / 该方法…）让句子"读起来更顺"。
  - **禁止拼接不同位置的片段。** 一条 anchor 只能来自原文**一个连续位置**；原文 A 处半句 + B 处半句拼成一条 = fabricated。
  - **禁止改大小写 / 标点 / 缩写。** 原文 `Qwen2.5-VL-72B` 就照抄，不许写成 `Qwen2.5 VL 72B`；原文怎么拼，你怎么拼。
  - **禁止补全被截断的句子。** 原文段尾断在哪就断在哪，不许把下一段开头接上来。
- **能 grep 到**：anchor 中间可有任意空白（会被归一化），但**首尾字符必须落在原文真实存在的位置上**。
...
输出 JSON 之前，用 Read 工具回到 `fulltext.txt`，对**每一条** anchor 逐条核对：这条 anchor 是不是从原文**某一个连续位置**直接复制下来的——首字符、尾字符、中间每个词，都和原文那一处一致，**没有你加的 `On` / `In` / `The` / `本文` / `我们` 之类**？只要有一条你无法在原文里指出它的确切出处，就把它换成你真正能逐字定位的那段，或删掉这条。**宁可少一条，不要错一条**——一条 anchor 过不了重算门，整条账作废、整期停线。
