---
name: podcast-studio-prep
description: "podcast-studio's internal prep orchestrator — invoked by the podcast pipeline's 达芬奇/davinci agent via orchestrator.py (script path), not a user-facing command. Two subcommands: 'check' returns a structured brief (approved topics + required angle + caller-supplied vault note + contrarian source) given candidate topics; 'finalize' MinHash-dedupes the final script against the past 7 days and writes topic_log on accept. Not when: you want the standalone personal-os podcast-prep — this is the vendored podcast-studio copy."
model: sonnet
allowed-tools:
  - Read
  - Write
...
cross-domain + self-past candidate pulls from the vault, and a small
reverse-source pool. Writer agents
...
达芬奇 picks a subjective note directly from `vault.subjective_dir` (resolved from
`~/.podcast-studio/config.yaml`) and passes the resulting `{id, title, excerpt}` object
as `--pkos-note` to `orchestrator.py check`. The orchestrator validates the note is
present but does NOT read the vault itself. If the note is missing, `check` returns
`{"error": "pkos_note required ..."}` and 达芬奇 must retry after selecting a note.
...
orchestrator.py check \
  ...
  --date=YYYY-MM-DD \
  --topic-log={output_dir}/topic_log.yaml \
  --pkos-note='{"id":"PKOS/note-id","title":"Note Title","excerpt":"brief excerpt"}' \
  [--show-type=morning|evening] \
  [--required-domains=tech,market,science,geo,culture] \
  [--force-domain=philosophy|cognition|history] \
  [--force-contrarian=lesswrong|marginal-revolution|stratechery] \
  [--seed=N] \
  [--vault-root=/path/to/PKOS]
```

Phase-2 flags (used by the podcast pipeline's davinci dispatch):
- `--show-type` selects the morning (event-centric, domain-quota) vs evening
  (spine-inverted, open-questions-primary) brief shape.
- `--required-domains` is the morning domain quota the selector must cover
  (caller-passed, never hardcoded).
- `--force-domain` / `--force-contrarian` pin this brief to one cross-domain
  bucket + reverse source, so the parallel A/B/C paths diverge at the brief layer.
...
`--vault-root` defaults to `vault.subjective_dir` in `~/.podcast-studio/config.yaml`
(resolved via `lib.config.load_config()`).
...
**Jaccard threshold**: 0.4 (`FINALIZE_DEDUP_THRESHOLD` in orchestrator.py; the old
0.15 is kept only as a fallback when a caller passes an explicit `threshold`). Scripts
more similar than 40% to any past-7-day script trigger retry. 快刀 must rewrite per the
returned `reason`.

## State File Locations

- **topic_log.yaml**: `{output_dir}/topic_log.yaml` (resolved via `lib.config.load_config()`)
  - `output_dir` is configured in `~/.podcast-studio/config.yaml` (under `vault:`)
  - Created automatically on first `finalize` accept
- **Script archive** (optional): a caller-supplied directory of past final scripts;
...
python3 orchestrator.py check \
  ...
  --date 2026-05-19 \
  --topic-log /tmp/topic_log.yaml
# Returns: {"error": "pkos_note required — caller must select a note from vault.subjective_dir ...", ...}
# 达芬奇 must select a subjective note and retry with --pkos-note.
```
...
```bash
# Returns: {"action": "retry", "jaccard": 0.87, "reason": "4-gram Jaccard similarity 0.8700 >= threshold 0.4"}
