...
# Sibling module (same scripts/ dir, on sys.path wherever cross_domain is imported).
...
def cross_domain_candidates(today_tags, vault_root, n=5, notes=None,
    ...
    # KL-4: cross_domain reads only the cross-domain directories named by the
    # vault's own directory contract (falls back to the PKOS defaults).
    cross_dirs = load_recall_dirs(vault_root)["cross_domain"]
    notes = [nt for nt in notes if nt["path"].startswith(cross_dirs)]
...
def same_topic_past_notes(today_tags, vault_root, days_min=7, days_max=90, n=5,
    ...
    if notes is None:
        notes = load_pkos_notes(vault_root)
    # KL-4: self_past reads only the self-past directories named by the vault's own
    # directory contract — viewpoints + past on-record stances (falls back to the
    # PKOS defaults: 20-Ideas/观点心得 + 90-Productions/Podcasts).
