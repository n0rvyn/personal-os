...
# DP-001 A: NO vault-read here — the PKOS note is supplied by 达芬奇 selecting a
# subjective note from vault.subjective_dir and passing it as `--pkos-note` argument.
sys.path.insert(0, str(Path(__file__).parent))
sys.path.insert(0, str(Path(__file__).resolve().parents[3]))  # podcast-studio/ — for `lib.config`
...
from lib.config import load_config, ConfigError  # Task 4-impl: podcast-studio config resolver
...
def _resolve_vault_root(explicit=None):
    """Resolve the podcast-studio vault root. Priority: explicit arg > PKOS_VAULT_ROOT env >
    config `vault.root` > config `vault.subjective_dir` (back-compat) > raise (no silent default).

    The vault root is the Obsidian/PKOS root — the base for recall and the
    directory contract (`<root>/99-System/10-Directory-Contract.md`). When
    `vault.root` is unset, falls back to `vault.subjective_dir` (the historical
    behavior). Cross-domain recall and self-past recall walk relative to it.
    """
    if explicit:
        return explicit
    ...
    if env:
        return env
    # Phase 1 / Task 4-impl: resolve from podcast-studio config. Fail-closed:
    # missing config / missing key raises ConfigError — no silent default.
    cfg = load_config()
    return cfg.vault.root or cfg.vault.subjective_dir
...
def run_check(candidates: list, topic_log_path: str, today: str,
    ...
    vault.subjective_dir). The orchestrator validates it's present but does NOT read the
    vault itself. If missing, returns an error brief that 达芬奇 must remediate (select a
    note + retry).

    The brief also carries cross_domain_candidates + self_past_candidates (pulled from the
    vault) and a named_concept_prompt directive. vault_root is resolved from
    the podcast-studio config (`lib.config.load_config()`) when not given explicitly.
    ...
    if not pkos_note or not isinstance(pkos_note, dict) or not pkos_note.get("id"):
        return {
            "error": "pkos_note required — caller must select a note {id, title, excerpt} from vault.subjective_dir and pass it as --pkos-note input. See podcast-prep SKILL.md for caller protocol.",
            "approved_topics": [],
            "pkos_note": None,
...
def main():
    ...
    chk.add_argument("--topic-log", default=None,
                     help="Path to topic_log.yaml. When omitted, defaults to "
                          "<config vault.output_dir>/topic_log.yaml (resolved via lib.config).")
    chk.add_argument("--pkos-note", default=None,
                     help="REQUIRED. JSON object {id, title, excerpt} selected by the caller (达芬奇) from vault.subjective_dir. DP-001 A.")
    chk.add_argument("--seed", type=int, default=None)
    chk.add_argument("--vault-root", default=None,
                     help="Podcast vault root. Defaults to vault.subjective_dir from the "
                          "podcast-studio config (~/.podcast-studio/config.yaml).")
    ...
    if args.cmd == "check":
        # Task 4-impl: --topic-log defaults from config vault.output_dir when omitted.
        if args.topic_log is None:
            cfg = load_config()
            args.topic_log = os.path.join(cfg.vault.output_dir, "topic_log.yaml")
