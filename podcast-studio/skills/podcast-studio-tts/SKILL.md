---
name: podcast-studio-tts
description: "podcast-studio's internal TTS step — synthesize the broadcast script to mp3 across Volcengine (Doubao) and MiniMax. Voice-id prefix routes the backend: volc-* → Volcengine, mm-* → MiniMax. Single-shot synth, long-text batched synth with auto-chunk + ffmpeg merge, quota-aware vendor selection; caller never writes vendor-specific curl. Invoked by the podcast pipeline (the 周杰伦/jay agent) via script path. Not when: you want standalone TTS unrelated to podcast-studio — use a general-purpose TTS skill."
allowed-tools:
  - Bash
  - Read
  - Write
...
- Pure text chunking with no synthesis — use `scripts/chunker.py` directly.
...
```bash
${CLAUDE_PLUGIN_ROOT}/skills/podcast-studio-tts/scripts/synth.sh \
  --text "你好测试" \
...
```bash
${CLAUDE_PLUGIN_ROOT}/skills/podcast-studio-tts/scripts/synth-auto.sh \
  --input transcript.md \
  --output podcast.mp3 \
