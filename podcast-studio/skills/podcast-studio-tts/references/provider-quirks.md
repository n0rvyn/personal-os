...
Staging dirs (`tts-batch-*` in `$TMPDIR`) are intentionally kept on failure for forensic inspection (see Threat Model in the plan). After a few weeks of TTS work with occasional failed runs, `$TMPDIR` accumulates stale staging dirs.
