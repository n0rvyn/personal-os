...
VENDORED COPY. Source of truth: pkos plugin, text-to-segments skill
(pkos/skills/text-to-segments/scripts/chunker.py), vendored at pkos v0.13.2.
Claude plugins cannot reference files outside their own directory once
installed, so tts-toolkit carries its own copy to stay self-contained.
Re-vendor from pkos if the upstream chunker changes.
