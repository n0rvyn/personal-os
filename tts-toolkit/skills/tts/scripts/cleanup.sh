...
#
# Identifies stale TTS staging dirs (tts-batch-*) in TMPDIR and stale wrapper
# backup files (*.bak.<timestamp>) under ~/.adam/roles/*/scripts/tts/.
#
# NEVER deletes audio files (.mp3 .wav .flac .pcm .m4a) anywhere, even inside
# staging dirs. Audio is an artifact, not temp.
#
# Usage:
#   cleanup.sh [--older-than DAYS] [--dry-run|--apply] [--scope all|staging|bak]
#
...
#
# Env overrides (for testing — avoids touching real /tmp or ~/.adam):
#   TTS_CLEANUP_TMPDIR      scan root for staging dirs (default: ${TMPDIR:-/tmp})
#   TTS_CLEANUP_ADAM_ROOT   scan root for .bak files (default: $HOME/.adam)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
Usage: $(basename "$0") [--older-than DAYS] [--dry-run|--apply] [--scope all|staging|bak]

Defaults: --older-than 7, --dry-run, --scope all

Scans for stale TTS temp files and either lists (dry-run) or deletes (apply) them.
NEVER deletes audio files (.mp3 .wav .flac .pcm .m4a) anywhere.

Scan roots (env-overridable for testing):
  TTS_CLEANUP_TMPDIR      staging dirs scan root  (default: \${TMPDIR:-/tmp})
  TTS_CLEANUP_ADAM_ROOT   bak files scan root     (default: \$HOME/.adam)
...
case "$scope" in
  all|staging|bak) ;;
  *) echo "cleanup: --scope must be all|staging|bak (got '$scope')" >&2; exit 1;;
esac

# --- Scan roots (env-overridable) ---
scan_tmpdir="${TTS_CLEANUP_TMPDIR:-${TMPDIR:-/tmp}}"
scan_adam_root="${TTS_CLEANUP_ADAM_ROOT:-$HOME/.adam}"
...
# Stage B — wrapper .bak.<timestamp> files under adam roles
if [[ "$scope" == "all" || "$scope" == "bak" ]]; then
  if [[ -d "$scan_adam_root/roles" ]]; then
    while IFS= read -r -d '' path; do
      [[ -f "$path" && ! -L "$path" ]] || continue
      bn="$(basename "$path")"
      # Must match *.bak.<digits> pattern
      [[ "$bn" =~ \.bak\.[0-9]+$ ]] || continue
      candidates+=("$path")
    done < <(find "$scan_adam_root/roles" -path '*/scripts/tts/*.bak.*' -type f -mtime "+$days" -print0 2>/dev/null)
  fi
fi
...
for candidate in "${candidates[@]}"; do
  ...
  # Refuse the live generate_voice.sh (only .bak.* siblings are targets)
  case "$candidate" in
    */generate_voice.sh)
      echo "skip (live script, only .bak siblings are cleaned): $candidate" >&2
      continue
      ;;
  esac
