#!/usr/bin/env bats
# test_cleanup.bats — 11 BATS tests for cleanup.sh operator-side temp-file cleanup.
...
setup() {
  ...
  mkdir -p "$TEST_TMPDIR/adam-root/roles"
  export TTS_CLEANUP_TMPDIR="$TEST_TMPDIR/my-tmpdir"
  export TTS_CLEANUP_ADAM_ROOT="$TEST_TMPDIR/adam-root"
}
...
# Helper: create a fake .bak file aged N days
make_old_bak() {
  local path="$1" days="$2"
  mkdir -p "$(dirname "$path")"
  echo "backup content" > "$path"
  local ts
  ts="$(python3 -c "import datetime; d=datetime.datetime.now()-datetime.timedelta(days=$days); print(d.strftime('%Y%m%d%H%M.%S'))")"
  touch -t "$ts" "$path"
}
...
@test "live generate_voice.sh is never touched, only .bak siblings" {
  local tts_dir="$TTS_CLEANUP_ADAM_ROOT/roles/周杰伦/scripts/tts"
  mkdir -p "$tts_dir"
  echo "#!/bin/bash" > "$tts_dir/generate_voice.sh"
  local ts
  ts="$(python3 -c "import datetime; d=datetime.datetime.now()-datetime.timedelta(days=20); print(d.strftime('%Y%m%d%H%M.%S'))")"
  touch -t "$ts" "$tts_dir/generate_voice.sh"

  make_old_bak "$tts_dir/generate_voice.sh.bak.1700000000" 20

  run bash "$CLEANUP" --older-than 7 --apply --scope bak
  [ "$status" -eq 0 ]

  # Live script must survive
  [ -f "$tts_dir/generate_voice.sh" ]
  # Backup must be deleted
  [ ! -f "$tts_dir/generate_voice.sh.bak.1700000000" ]
}

@test "scope=staging skips bak files" {
  make_old_staging "$TTS_CLEANUP_TMPDIR" "tts-batch-SCOPE1" 10
  local tts_dir="$TTS_CLEANUP_ADAM_ROOT/roles/testrole/scripts/tts"
  make_old_bak "$tts_dir/generate_voice.sh.bak.1700000001" 10

  run bash "$CLEANUP" --older-than 7 --apply --scope staging
  [ "$status" -eq 0 ]

  # Staging dir deleted
  [ ! -d "$TTS_CLEANUP_TMPDIR/tts-batch-SCOPE1" ]
  # Bak file untouched
  [ -f "$tts_dir/generate_voice.sh.bak.1700000001" ]
}

@test "scope=bak skips staging dirs" {
  make_old_staging "$TTS_CLEANUP_TMPDIR" "tts-batch-SCOPE2" 10
  local tts_dir="$TTS_CLEANUP_ADAM_ROOT/roles/testrole2/scripts/tts"
  make_old_bak "$tts_dir/generate_voice.sh.bak.1700000002" 10

  run bash "$CLEANUP" --older-than 7 --apply --scope bak
  [ "$status" -eq 0 ]

  # Staging dir untouched
  [ -d "$TTS_CLEANUP_TMPDIR/tts-batch-SCOPE2" ]
  # Bak file deleted
  [ ! -f "$tts_dir/generate_voice.sh.bak.1700000002" ]
}

@test "non-existent scan roots produce no output and exit 0" {
  export TTS_CLEANUP_TMPDIR="$TEST_TMPDIR/nonexistent-tmp"
  export TTS_CLEANUP_ADAM_ROOT="$TEST_TMPDIR/nonexistent-adam"
