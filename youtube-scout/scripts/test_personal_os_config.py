"""pytest tests for personal_os_config.py (youtube-scout flat base copy).

Mirrors domain-intel's test layout — the 4 base copies share the same grafted
anchor unit and must accept the same equivalence-shield assertions.
"""
import os
import subprocess
import sys
...
class TestResolveConfigPath:
    ...
    def test_home_fallback_byte_identical_to_legacy_load(
        ...
        """Regression shield: load_config() byte-identical to pre-Phase-2 flat derivation."""
        import importlib
        sys.path.insert(0, str(SCRIPTS_DIR))
        import personal_os_config
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
